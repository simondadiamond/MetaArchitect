...
| 2026-06-27 | `agent-job-background` spawned sitemaster no-op'd: `/app/data/projects/` is NOT mounted into spawn-mode containers, only chat-mode. Sitemaster correctly refused stale handoff bundle but had no autonomous fallback — empty PR #91 (Sterling) auto-merged. | Idempotent `agents/sitemaster/scripts/bootstrap-repos.sh` (clone-or-symlink, exits 50ms if already populated) is now mandatory Step 0 of every sitemaster job per `agents/sitemaster/SYSTEM.md`. Failure → DM Simon, never silent no-op. Pattern hoists to `agents/CLAUDE.md` when a second agent needs it. |
| 2026-06-28 | Static code review on PR #35 (sitemaster's RICE redesign) rated all components elite — but the live preview walk surfaced a `text-4xl` × `w-16` column overflow making `149.3` and `76.8` clip into the title. Bug invisible in static review; only manifests under real ≥4-char data widths. | A live walk is the closing gate on every UI-touching PR, not nice-to-have. If preview walk is blocked (no Vercel bypass token, Supabase allow-list, etc.), use the localhost-dev fallback in `agents/sitemaster/CLAUDE.md` — never skip the walk and substitute "deeper code review." Pushed `773bed7` widening to `w-24`, re-walked, merged at `94d52c8`. |
