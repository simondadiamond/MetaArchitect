...
(branch `draftsman-rollout`) — but `git commit` was denied by
`scripts/hooks/bash-guard.sh` Rule 6. Root cause: the `mentions_primary` check was an
...
`.../projects/simonparis-website-draftsman` satisfied the pattern (the trailing group
was optional), even though the anchored `cwd`-side check correctly did NOT treat this
worktree as the primary checkout. Reproduced directly by the controller, not just the
subagent, then verified against 3 true-positive cases before applying.

**Fix applied and merged (PR #102):** anchored on a `/`, whitespace, or end-of-string
boundary after each `$PRIMARIES` alternative, matching how the `cwd`-side check is
already anchored:
...
`scripts/hooks/test-hooks.sh` — 69/69 pass. Sitemaster re-dispatched to resume at
Task 4 Step 3 (the file is already built and staged — just needs the commit) and
continue the SDD ledger at
...
2. **Tasks 0–2 are DONE** (Fable/CMO session 2026-08-09, merged to main): plugin check all-PASS (49 skills, symlinks good, marketplace current); the operator visual system lives at `brand/visual-operator.md` (the design authority for builder prompts; practitioner counterpart `brand/visual-practitioner.md`; brand-summary §"Visual Identity — Lane-Routed" routes between them — load one lane, not both); the copy audit is at `funnel/setup-offer/copy-audit-2026-08-09.md` — apply its CURRENT→REPLACE pairs verbatim. **Start at Task 3** (worktree setup).
...
- **Order:** docs-first — plugin check (Task 0), brand-summary v-next (Task 1), copy audit (Task 2) BEFORE any website code. `brand/visual-operator.md` (Task 1 created it; split out of brand-summary 2026-08-09) is what every builder prompt cites.
...
- **Tasks 4–11 (build):** dispatch sitemaster-profile subagents; every prompt includes: the plan task verbatim, `brand/visual-operator.md`, and the don't-list. Review each page against the reference mockup + orange-budget/no-mono checks before starting the next.
