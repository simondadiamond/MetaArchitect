...
**Fix applied:** teardown-research gate broadened to full claim provenance (numbers + process narratives + attributed statements; conclusions-from-silence are ours). Candidate row purged same session. Proposed to Simon (design pending): deterministic source fetch in the teardown runner — snapshot each source's text into a `pipeline.teardown_source_snapshots` table and build the prepare prompt from the snapshots, making every teardown debuggable against its actual evidence.
