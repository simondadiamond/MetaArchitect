...
**Fix applied:** Killed orphan, restarted orchestrator; workers connected and the pending workflow published immediately (~59 min late, releaseURL captured, updatePost completed). linkedin-publish SKILL.md failure paths updated with the no-worker diagnosis + correct restart procedure. **Owed (needs Simon's go):** a watchdog — if Temporal is healthy but the `linkedin`/`main` task queues have no pollers, `docker restart postiz` and ping ntfy. Without it, every host reboot re-arms this failure.
