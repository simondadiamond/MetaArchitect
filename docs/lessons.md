...
**Where documented:** This entry; memory feedback_no_broad_pkill.md.
...
**What happened:** The first /weekly-brief run put "post the 6 drafted engage replies" at rank 1. The drafts were `engage_posts.toplevel_draft` rows on posts from Jul 7–9 — the oldest 4 days stale. LinkedIn reply reach decays within a day or two (the sweep lands at ~07:00 UTC, so opportunities are already ~16h old at first sight; one missed day kills them). Simon judged the batch not worth posting and skipped everything — the brief's top task produced zero action.
**Root cause:** Two gaps. (1) No freshness rule anywhere in the chain: `engage_posts` has a `stale` status in its vocabulary that nothing ever set, and neither the brief's exploration nor engage-replies filtered by `posted_at` age. (2) Skip-state blindness: Simon's skips persist in `engage_comments.status`, but the brief counted a different surface (`engage_posts.toplevel_draft`) that has no skip mechanism — so once he skipped, nothing told the next brief the inventory was dead.
**Fix applied:** engage-replies SKILL.md: 48h freshness gate — posts older than 48h are marked `stale` and never presented; 24–48h presented with age shown. weekly-brief SKILL.md: "domain facts" block — engagement is perishable; actionable engage inventory = `engage_comments` status `new` + `engage_posts` `mined` within 48h; skipped/engaged/stale is done, never backlog. Story queued: sweep auto-stales `engage_posts` rows older than 48h so hygiene is mechanical, not skill-dependent. Corrected brief row inserted without the dead task.
