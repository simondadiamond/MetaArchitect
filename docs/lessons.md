...
## 2026-08-14 — The night-build lane ran ~11 consecutive nights and built nothing, silently

**What happened:** The nightly 1:00am `/night-build` schedule (`aca758d5`, enabled, last run 2026-08-14T01:00) has been firing correctly for over a week and selecting nothing. 16 goals carry `agent_eligible = true` + `status = pending`, but **zero** have the `acceptance_criteria` **column** populated — so the night-ready set has always been empty. Seven of them have fully-written acceptance criteria sitting inside `description` as a `## Acceptance criteria` markdown section, which the selector doesn't read (and must not interpret). Nobody noticed because the skill's correct "nothing night-ready → exit silently" invariant makes a permanently dry lane indistinguishable from a quiet one.

**Root cause:** Two instructions that never met. CLAUDE.md behavior #9 tells the COO to flag ideas `agent_eligible=true` **at capture time** (scouting), while the column that actually gates the lane is only written **at scoping time** by the `grilling`/`to-tickets` ritual. Mid-chat capture therefore produces goals that *look* night-ready in the table and are invisible to the selector, and nothing in any weekly surface ever pulled a flagged goal through scoping. A capture flag was doing duty as a readiness flag.
