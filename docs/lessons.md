...
## 2026-07-16 — STATE Field Guide shipped a claims table that was never independently checked, including a category-error citation

**What happened:** The STATE Field Guide PDF (flagship lead magnet) shipped and merged to main carrying every stat and the full "Gap Matrix" competitor-comparison table straight from `brand/state-framework.md`, treated as ground truth because it was the designated brand source-of-truth file. Simon asked directly whether the stats were verified. A first pass caught one wrong stat (a "90 days median rebuild cycle" claim that misrepresented a Cleanlab survey and invented a causal link to the Reboot Test that the survey never made). A second pass, specifically fact-checking the six-framework Gap Matrix, found that 5 of 6 rows had errors — including "DataStates-LLM," a real citation, but for an HPC *training*-checkpoint research paper (arXiv:2406.10707) with zero relevance to agent/production LLM reliability, sitting in a table next to LangGraph/LangSmith/Arize/Weave/OTel as if it were a comparable competing framework. The matrix's own summary callout ("Auditable is unowned by every framework here — nonexistent") directly contradicted Arize Phoenix's own row in the same table, which rated Auditable as Partial — a self-contradiction present in the original document that the first two render-QA passes both missed because QA checked layout/voice/brand-color, not factual accuracy.
**Root cause:** Brand/source-of-truth files were treated as pre-verified fact because they're the designated "source of truth" for content generation, not because anyone had checked their claims against primary sources. Nothing in the render-QA loop (3 passes, all visual/layout/voice-focused) included a fact-check pass, so an unverified table could pass all three "iteration passes" and still ship wrong.
**Fix applied:** Independently verified every numeric stat via WebSearch/WebFetch against primary sources (Cleanlab report, arXiv, Quebec Law 25) and every Gap Matrix cell via 6 parallel research agents against each framework's official docs/repo/spec. Corrected `brand/state-framework.md` (both the Tolerant-pillar Evidence line and the full Gap Matrix, with per-row verification notes) and the PDF itself, so the same errors can't get reused in future content generated from that file. DataStates-LLM removed from the matrix entirely rather than re-rated, since it isn't a fair peer at all.
**The generalizable rule:** a brand/reference doc being the designated "source of truth" for *voice and structure* does not make it verified for *facts* — anything in it presented as an external stat, citation, or competitive claim needs an independent primary-source check before it ships in a public-facing deliverable, the same way a code claim needs a test. Layout/voice QA passes do not substitute for a fact-check pass; they check different failure modes.
**Where documented:** This entry; `brand/state-framework.md` Gap Matrix section; `funnel/lead-magnets/state-field-guide.html`.

## 2026-07-17 — Bench read a correct synonymous answer (CPA vs accountant) as a recall regression

**What happened:** After the sources-vault sync embedded a backlog note, the brain recall bench dropped to 12/15. The "new failure" was the Valerie question returning the newer CPA/external-auditor note — a *correct* answer — while the bench's `must_contain` demanded the literal word "accountant". Simon spotted it immediately ("cpa is somewhat synonymous with accountant").
**Root cause:** Literal substring assertions can't tolerate semantically equivalent phrasing, so a richer correct answer reads as a regression. Occupation/role words are the risk class; proper nouns and exact values are fine as literals.
**Fix applied:** `bench/run.mjs` `contains()` now accepts `|`-separated alternatives per needle; the Valerie question uses `"accountant|CPA"`. Bench back to 13/15 (then 15/15 — next entry). One-liner appended to goal 139c69d3.
**The generalizable rule:** when a bench fails, first ask whether the answer is wrong or the assertion is. Eval assertions on natural-language output need synonym tolerance for role/category words, or they punish improvements.
**Where documented:** This entry; `~/projects/brain/bench/run.mjs` (comment on `contains`); goal 139c69d3.

## 2026-07-17 — Keyword-dense note slug shadowed sibling notes in brain find; cross-links fixed it

**What happened:** The two "known" bench fails (sterling IPs, N8N_PROXY_HOPS) both traced to one note — `n8n-on-sterling-binds-to-the-tailscale-ip` — whose slug packs sterling+tailscale+ip and outscored both the sterling hardware note and the warning-suppression lesson. A proposal to fix this had been filed by an earlier session and then sat; the actual fix took minutes once attempted.
**Root cause:** Slug substrings score highest in lexical ranking, so a keyword-dense slug lexically shadows its siblings; and a filed-proposal was treated as progress when the fix was small and deterministic.
**Fix applied:** Cross-links added to the n8n note body (`[[sterling-hp-prodesk-400-g5-home-server]]`, `[[a-suppressed-security-warning-is-not-a-closed-exposure]]`) so find's pointer-hop appends the right evidence — bench 15/15 for the first time (brain 464a91b). Brain CLAUDE.md rule 4 now requires cross-linking at save time when a new note shares 2+ slug keywords with an existing note. Small deterministic fixes get applied in-session, not parked as proposals (coo memory).
**The generalizable rule:** in lexical retrieval, hub-ish documents win queries they can't answer; the structural fix is links that route the hop to the right evidence, not score tweaks. And a proposal is not a fix.
**Where documented:** This entry; `~/projects/brain/CLAUDE.md` rule 4; brain commit 464a91b.

## 2026-07-19 — Customer-facing web copy shipped with AI tells and an exposed internal mechanic

**What happened:** The /setup landing page shipped to production carrying 22 EN / 23 FR em dashes and a bonus block labeled "Built once, included for every client" — the internal make-once economics of the offer, visible to the customers it was priced for. Simon caught both on a live read ("theres a lot of em dashes.. can we humanize this"; "the whole build once thing was not meant to be seen by customers"). A dedicated fix PR (#84) was required.
**Root cause:** The em-dash/AI-tell gates lived only in the content-pipeline path (editorial, insert-blog-post, linkedin-playbook). In-session web copy had no gate, so the same writer that respects the budget inside pipeline.posts poured em dashes into page copy unchecked. And the make-once framing was a business-strategy fact that migrated into customer copy because the same session held both contexts.
**Fix applied:** Copy rewritten to zero em dashes in both locales (PR #84); bonuses renamed as customer-value deliverables. Root-cause rule added to `brand/brand-summary.md` prohibitions: AI-tell gates apply to ALL customer-facing surfaces, and internal business mechanics never appear in customer copy. One-liner appended to goal 2116b881.
**The generalizable rule:** voice gates scoped to one output path will be bypassed by every other path that produces prose; scope gates to the audience (customer-facing), not the pipeline. Business-mechanic framing ("built once, sold N times") is for the operator's docs, never the buyer's page.
**Where documented:** This entry; `brand/brand-summary.md` prohibitions; goal 2116b881.

## 2026-07-19 — Chained compound bash silently killed by a hook denial and a swallowed build failure

**What happened:** Twice in one session a long `&&` chain died invisibly. First: commit+push+PR+merge chained with `gh pr merge` by branch name — the PreToolUse hook denied the merge pattern, which blocked the ENTIRE chain before anything ran (the commit itself never happened; only a later error surfaced it). Second: build+server-start+verify chained so that when `grep` on the build output matched nothing, the `&&` chain stopped, the server never started, and the verify greps returned empty — misreading as "content missing" when the real state was "server not up".
**Root cause:** Hooks match on the whole compound command, so one guarded segment vetoes unrelated work packed into the same call; and `&&` chains make a mid-chain empty grep indistinguishable from a real failure downstream.
...
## 2026-07-20 — Panel teardowns were an island: no handoff to the blog/LinkedIn pipeline, and it was "confirmed" without testing

**What happened:** Simon completed his first teardown through the upgraded Command Center training panel (Morgan Stanley). He had earlier asked whether a teardown creates a blog draft in `/content` and LinkedIn posts, and an agent confirmed it would — without testing. It didn't: the panel writes `pipeline.teardown_drafts` only. The blog/LinkedIn handoff (blog_ideas row at `editing` + draft artifact + `pipeline.posts` announcement) exists only in the session-run `teardown-generate` skill (Steps 4/4b); the panel runner never calls it. The 2026-07-06 distribution-fanout spec that would cover this was drafted but never implemented — no `distribution_jobs` table, no `dist-*` schedules.
**Root cause:** Two pipelines with the same name ("teardown") — the panel engine and the Content-Engine skill — share output shape but not downstream wiring, and the confirmation answered from the skill's docs instead of tracing what the panel actually writes.
**Fix applied:** Manual handoff run for Morgan Stanley (blog_ideas `d8f1e99c` at `editing`, draft artifact saved, `pipeline.posts` announcement `e7df450c` with score-card media); dispatcher fired via Run-now. Root-cause story queued to add the handoff to the panel's completion path.
**The generalizable rule:** "X should happen automatically" is only true if you can name the exact schedule/trigger that fires it and the row it picks up. Confirmations about pipeline behavior must cite the trigger, or be reported as untested.
**Where documented:** This entry; story queued in command-center.

## 2026-07-21 — Command Center stayed dead for 15h: a swallowed signal defeated `Restart=on-failure`

**What happened:** Command Center (port 3737) went unreachable at 14:09 UTC and stayed down until Simon noticed and asked for a fix. The systemd unit showed `Main PID ... code=exited, status=0/SUCCESS` — a "clean" exit — so `Restart=on-failure` correctly did nothing. Sibling processes (worker, term-daemon) kept running the whole time; only the Next.js child died.
**Root cause:** `bin/start-cc.js` spawns `next start` and forwards its exit via `child.on("exit", (code) => process.exit(code ?? 0))`. When a child dies by signal (SIGKILL from OOM, `kill -9`, a crash), Node reports `code: null` — the `?? 0` fallback silently reported that as a successful exit to systemd. Whatever actually killed the process left zero trace anywhere checked (kernel OOM, earlyoom, systemd-oomd, cgroup accounting, logind/session) — consistent with a SIGKILL but the specific trigger could not be identified; auditd isn't installed on this box so signal delivery isn't traceable after the fact.
**Fix applied:** `child.on("exit", (code, signal) => ...)` now logs the signal and exits 1 on any signal-based death, so systemd sees a real failure and `RestartSec=5` kicks in. command-center PR #123 (merged to main; deploy-sync auto-deployed).
**The generalizable rule:** any process-supervisor wrapper that re-exits with a child's exit code must not collapse a null (signal-death) code into 0 — that's indistinguishable from success to systemd's `Restart=on-failure` and silently disables crash recovery. Check every spawn-and-forward wrapper for this pattern, not just this one.
**Where documented:** This entry; `bin/start-cc.js`. Sterling's tight memory headroom (`~/.claude/projects/.../memory/project_sterling_memory_headroom.md`) remains the leading suspect for the trigger but is unconfirmed — no OOM subsystem logged a kill.

## 2026-07-21 — Editing an optimized_draft after fact-check strands the row at insert (stale-report gate)

**What happened:** Final-review text fixes to the Morgan Stanley teardown were saved as new `optimized_draft` artifacts AFTER the factcheck_report was written. blog-insert's stale-report precondition (report older than newest draft → fail) correctly refused to publish, but the failure surfaced as repeated "keeps failing" approves for Simon: the CC retry re-ran insert against the same stale report every time.
**Root cause:** The editing session (COO) knew the fact-check had passed and treated post-factcheck edits as free. The pipeline's contract is that a factcheck_report describes one exact text; any new draft artifact invalidates it. Nothing reset the stage to `fact_check`, so the row sat at `awaiting_final_review` pointing at a report that no longer described the draft.
...
## 2026-07-28 — Verify guard rejected identical screenshots that were the correct proof of a stasis criterion

**What happened:** Story 6431714e (/setup hero "The Ask" animated window) parked at `verifying` after 3 fix attempts — with correct, fully-passing code. Its criteria include "with prefers-reduced-motion the final frame renders statically"; the verifier proved it with two screenshots 300ms apart that were byte-identical. The worker's `identical_screenshots` evidence guard (added after lesson 2026-07-12, where identical shots were fraudulent proof of scrolling) rejected both honest passing verifies on that exact pair. Fix attempt 1 and 3 fixed real bugs (contrast, cursor blinking under reduced motion); attempt 2 correctly concluded "no code defect" — but each fresh verify session re-captured the identical pair and got rejected again, burning the budget.
**Root cause:** The guard encoded "identical evidence = nothing happened = bad" unconditionally, but stasis criteria invert the meaning: identical timed captures are exactly what a static frame looks like. A deterministic backstop written against one fraud case had no channel for the verifier to say "this group is identical BY REQUIREMENT."
...
## 2026-07-31 — Stale failing verify report in a PR body read as "the story errored out"

**What happened:** Voice story 34953e91 (CC PR #135) failed verify criterion 6 (every reply spoken twice — double `done` chunk from `/api/voice`). The fixer repaired it (b79a042) and the re-verify passed 12/12 (`.story/result.json` at c5e18d6) — but the worker never refreshed the PR body, which still showed the first run's "11/12, criterion 6 FAIL". Simon read the PR as errored-out; a session had to trace commit timestamps against the result artifact to establish the branch was actually green, then re-verify live (two `/api/voice` turns, exactly one `done` each) before merging.
**Root cause:** The worker posts the PR body once from the verify report available at PR-creation time; a later fix→re-verify cycle updates `.story/result.json` on the branch but not the PR body, so the PR's human-facing surface can permanently disagree with the branch's machine-facing truth.
...
## 2026-08-04 — `apt autoremove --purge` swept the entire nodejs runtime off Sterling

**What happened:** At 12:25:23 UTC a session ran `sudo apt autoremove --purge` on Sterling (dpkg log: `Requested-By: diamond`). It purged `nodejs` itself plus its full transitive dependency tree (`node-readable-stream`, `node-inherits`, every `node-*` package). Node had been apt-mark'd manual already, so this wasn't a plain "orphaned auto-dependency" cleanup — most likely a conflict-resolution side effect during the same autoremove run (half-configured/half-installed states in dpkg.log around the same timestamp point at a broken package interaction, not simple orphan sweep). Effect: every node-based automation on the box went dark simultaneously — `blog-pipeline-dispatch`'s scheduled fire (this session) found `node: command not found`, and the same breakage would hit any other schedule, skill, or MCP tool that shells out to `node`/`npm` until the next thing that happened to reinstall it.
**Root cause:** `apt autoremove --purge` on a shared dev box is not scoped to "packages I just uninstalled" — it re-evaluates the entire dependency graph and can take out load-bearing runtimes as a side effect, even ones marked manually-installed, when a conflict-resolution path is involved. Nothing in this repo's guardrails treats system package removal as a risk class; the mechanical hooks (`scripts/hooks/`) only cover git/file mutations in specific checkouts.
...
**Root cause:** WebFetch answers a prompt against a page using a small fast model. Asked "does X exist?", it reports absence when the *excerpt it saw* lacks X — and phrases that absence as a finding ("Key Finding: …cannot be…"). Absence of evidence was consumed as evidence of absence, and the confident phrasing suppressed the instinct to check. Compounding it: the same session had *correctly* verified the analogous MailerLite limitation by hitting the live API, so the negative result felt consistent with an established pattern rather than suspicious.
**Fix applied:** Spec rewritten (rev. 2026-08-05) with a correction notice in §2 recording the error and its cause, so the next reader doesn't re-derive the wrong architecture from the old premise.
**The generalizable rule:** a *negative* claim about an API's capabilities — "no endpoint for X", "cannot be done via API" — is never established by a doc summariser. Probe the actual surface: `curl -o /dev/null -w "%{http_code}"` against the plausible endpoint paths, or grep the raw `llms.txt`/OpenAPI spec rather than asking a model to summarise it. This matters most precisely when the answer is load-bearing: if a capability's absence is about to shape an architecture, the cost of a 10-second probe is nothing against the cost of building a subsystem to route around a limitation that doesn't exist. Positive claims are cheap to trust (the thing either works or errors at build time); negative claims silently expand scope.
**Where documented:** This entry; `docs/superpowers/specs/2026-08-04-email-infrastructure-design.md` §2.
**Related:** the 2026-08-06 Supabase-migrations entry below is the same failure family from the other direction — there, a repo doc's claim about a limitation went unprobed. Same rule: a claim that something *cannot* be done expires, and the cost of checking is always lower than the cost of designing around it.
...
**Correction (same day, 2026-08-06):** the entry above led me to a *false negative*. Having established that Resend silently drops malformed writes, I concluded from four failed experiments that automation emails simply cannot be personalized — and recommended redesigning the architecture around that. Simon pushed back and told me to research it instead of trusting my tests. The capability exists; I had the syntax wrong. A `send_email` step resolves a template variable through a **structured reference**, `{"SCORE": {"var": "contact.properties.quiz_score"}}` — not a mustache string. Every mustache form I tried (`{{x}}`, `{{{x}}}`, `{{event.x}}`, `{{contact.properties.x}}`) is passed through to the template as literal text, which is the same silent-success failure, so each negative result *looked* like proof of absence. One page of documentation (`resend.com/docs/dashboard/automations/send-email`) settled in a minute what four experiments had gotten backwards.
**The rule this adds:** black-box probing proves what a *specific payload* does, never what the API *supports*. Failed guesses at an unknown contract are evidence about the guesses, not about the capability. When several attempts at a documented-but-unread feature fail the same way, that is the signal to go read the contract — not to redesign around the limitation. Probing and reading are complements: probe to verify a claim, read to learn the shape. The 2026-08-05 entry warned against trusting a summariser over a probe; this is the inverse error, and both come from the same root — treating one cheap source as sufficient when the answer is load-bearing.

---

## 2026-08-06 — A secret leaked through a database ERROR MESSAGE, from a script written to keep it out of the transcript

**What happened:** Registering the Resend webhook, my script took the signing secret straight from the API response and wrote it into Supabase Vault without ever printing it — the whole point being that the value never reaches the transcript. It built the SQL with `json.dumps(secret)`, which emits **double** quotes. Postgres reads a double-quoted string as an *identifier*, so the query failed with `column "whsec_..." does not exist` — and the error body, printed as a diagnostic, contained the secret in full. The script's only output line leaked exactly what it was designed to protect. Third credential exposure on this project (MailerLite key via a Python traceback 2026-08-04; that cleanup re-leaked them 2026-07-13).

**Root cause:** Two mistakes compounding. (1) `json.dumps` is not a SQL quoter — it looks like escaping, and for JSON it is, but SQL string literals are single-quoted and `''`-escaped. (2) The error path printed a provider response verbatim. Secret hygiene was applied to the *success* path only; nobody asks "what does this print when it fails?" — and the failure path is precisely where systems echo your input back at you.

**Fix applied:** Rotated immediately — deleted the webhook, which invalidates that signing secret, then re-registered. Added a `sql_literal()` helper (single quotes, `''` escaping) and made the error handler run `.replace(secret, "[REDACTED]")` **before** any output, so a different failure mode cannot leak it either.

**The generalizable rule:** a secret is not safe because the happy path never prints it. Audit the **error** path, because that is where an API or database hands your input straight back to you. Two mechanical habits: build SQL literals with a real quoter (never a JSON serialiser), and redact known secret values from any provider response before printing it, unconditionally — not only where you expect them to appear. Treat "this string is a credential" as a property that follows the variable into every output path, including stack traces and 4xx bodies.

**Where documented:** This entry. Pattern also relevant to `scripts/scrub-transcripts.py`, which exists because of the earlier instances.
