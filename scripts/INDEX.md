...
- prospect-mine.mjs — mine LinkedIn comment sections of AI-for-business creators (harvestapi, cookie-free, ~$10/run) for setup-venture prospects; scores owner-headline commenters, dedupes vs CRM, inserts via :3737 API with mined comment as timeline note; sources in prospect-mine-sources.json, filters are hypothesis per funnel/setup-offer/icp.md; --dry-run, cached stages (born 2026-07-20)
...
- insert-test-intake.mjs — insert a fixture /readiness row byte-faithfully to the website form (anon key, ?select=id only) for RLS/form regression tests; prints row id; cleanup (row + pipeline.logs markers) is on you (born 2026-07-19)
...
- price-drift-check.sh — hourly-rate drift guard: flags $/hr amounts in funnel/ + brand/ that aren't in simonparis-website lib/pricing.ts (source of truth; falls back to primary checkout from worktrees); per-line opt-out marker `price-ok` for comps/derived math; red-green fixture-tested (born 2026-07-31, lessons.md 2026-07-29 price fabrication)
- goals-crud.mjs — goals table CRUD: `create '<json>'` via Command Center API, `get/append/patch <id>` via Supabase REST; replaces inline python/urllib heredocs (born 2026-08-04)
