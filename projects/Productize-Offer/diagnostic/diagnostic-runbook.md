...
- [ ] Intake submitted → the analyzer auto-runs within ~15 min (`tools/intake-watch.mjs`, Command Center schedule) and ntfy-pings with the artifact path — provisional scorecard, call brief, memo skeleton in `~/engagements/<system>-<rowid8>/`. If the ping says FAILED (or never arrives), rerun manually: `node projects/Productize-Offer/tools/intake-analyzer.mjs --row <uuid> --model claude-sonnet-5` from repo root — always pin the model; the CLI's headless default can resolve to Haiku, which fails the quote-verbatim gate more often (lessons.md 2026-07-17).
