...
- [ ] Intake submitted → run the analyzer: `node projects/Productize-Offer/tools/intake-analyzer.mjs --row <uuid> --model claude-sonnet-5` from repo root (spec: `../intake-analyzer-spec.md`); it writes the provisional scorecard, the call brief, and the memo skeleton to `~/engagements/<system>-<rowid8>/`. Pin the model for engagement runs — the CLI's headless default can resolve to Haiku, which fails the quote-verbatim gate more often.
