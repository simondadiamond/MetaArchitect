...
> readout call). Numbers marked ⚠ are proposals — Simon confirms before first delivery.

## What the buyer gets

A written verdict, not hours. The calls are intake and delivery; the **document is the
product**.
...
- **Audit + Roadmap document** (⚠ 5–8 pages), containing:
  ...
  4. **Quoted Setup scope** — business-type variant, the 3 library skills, custom-skill
     candidates (≤2), named connectors
- **60-min readout call** walking the document
- **$2,500 credited in full** against OS Setup if they proceed (risk reversal — lead
  with it)

## Delivery flow

1. **Discovery call (90 min)** — drive it with the kit's `runbook/discovery-map.md`.
   Book the readout before hanging up.
2. **Async review** (⚠ 2–3 working days) — tool-stack walkthrough, sample artifacts
   they share (exports, screenshots, forwarded threads). No credentials, no installs —
   read-only by design.
3. **Write the document.** Speed is margin; never pad. Fast delivery at fixed price is
   the offer working as designed.
4. **Readout (60 min)** — verdict first, then the roadmap, then the Setup quote. Close
   with the credit mechanic.

⚠ Elapsed promise: **inside one week** from discovery call to readout.

## Boundaries

- **No building during an audit.** Anything hands-on-keyboard is a Working Session or
  Setup scope. The audit is eyes and judgment only.
- **Setup is only ever quoted from an audit** (ladder rule). No audit → no Setup quote.
- **Not a discounted consulting bucket.** If the buyer knows exactly what they want
  built, route to Working Sessions instead — the audit is for "I don't know what to
  build first."
- Deliverable is theirs to keep regardless of whether they proceed. Say so in the pitch.

## Segue script (demo → audit)

After the live demo run on Simon's rig:

> "Everything you just watched runs on a base — memory, a business profile, connectors,
> guardrails. The skill is an app sitting on that operating system; installed alone on
> a bare machine it does a third of what you just saw. Installing that base is my main
> offer — and we always start with an audit: I spend a few days mapping how your
> operation actually runs, and you get a ranked roadmap of what's worth automating,
> what isn't, and what it would cost. $2,500, credited in full to the setup if we go
> ahead. You keep the roadmap either way."

## Pricing logic (own the sentence)
