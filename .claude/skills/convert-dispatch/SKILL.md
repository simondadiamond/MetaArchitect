...
**`x`** — for each saved EN LinkedIn draft, write one cross-post variant (≤ 280 chars, or a 2–4 tweet thread for bigger stories). Variants are **stored in the conversion's `result.x_variants` only** — no X channel exists yet. (When Postiz gains a connected X integration, the linkedin-publish tooling handles scheduling; this skill still just stores variants.)
