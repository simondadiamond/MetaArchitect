...
**What changes vs. interactive mode — exactly three things:**
1. **Discovery replaces Step 2's explicit source.** Query `blog_posts` for `status='published'` rows, newest first. A post is already processed if `pipeline.logs` has a row with `step_name = 'repurpose_auto_processed'` AND `entity_id = <blog_posts.id>` — an exact structured match, never a substring search of prose summaries (the pre-2026-07-15 slug-in-`output_summary` convention broke silently if the format drifted). Process **at most one unprocessed post per fire** (bounded runs; the cron clears any backlog across fires). No unprocessed posts → log a `repurpose_auto_sweep` entry (`output_summary: 'no unprocessed published posts'`) and exit silently.
...
3. **Close out the post with the processed marker + ntfy (both mandatory).** After the post's candidates are done — whether N drafts were saved or every candidate was dropped — write the dedup marker via `logEntry({ workflow_id: state.workflowId, entity_id: <blog_posts.id>, step_name: 'repurpose_auto_processed', stage: 'complete', output_summary: '<slug>: <N> drafts saved, <M> dropped', model_version: '<model id>', status: 'success' })`. Writing it on the all-dropped path too is what bounds the sweep — without it the same post is re-attempted every fire forever. Then, only if N > 0, ping Simon: `import { ntfy } from './tools/supabase.mjs'; await ntfy('<N> LinkedIn drafts ready to review — <slug> — CC → Content → Social')`. The marker is written BEFORE the ping (a dead ntfy must never cause a re-run); a false return from `ntfy()` gets one line in the run record, nothing more.

**Candidate counts (Step 3/4 override):** `post_type = 'article'` → 3 candidates; `post_type = 'teardown'` → **3** (angles to pick from: scored-receipts, gap-found, pillar-by-pillar, what-they-got-right). Same rule as ever: each candidate a different angle, different hook pattern. Teardowns are capped at 3 because the teardown's own announcement post (its `linkedin_post`, saved to `pipeline.posts` at handoff) is already the cycle's first post — 1 + 3 matches the fan-out spec's target of 4 total, floor 3 (Simon's call 2026-07-20: 1 + 4 was too much repurposing of one source). The announcement's angle is TAKEN: treat that row's text as an already-used angle the candidates must complement, on top of the Step 5 reuse checks.

**Teardown auto-carousel (Simon's ask 2026-07-20):** when the processed post is `post_type = 'teardown'`, the run also builds the carousel — after Step 7 saves the candidates, before the item-3 close-out. Source: the `pipeline.teardown_drafts` row whose `blog_slug` matches the post's `slug` (none found → treat as a carousel failure per item 4 below, never fail the whole run). Run Carousel Mode C1 → C3 → C5 → C6 with these deltas:

1. **C4 (present-and-stop) is skipped** — same principle as Step 6's replacement: the C3 manifest gate plus C5's mandatory per-PNG visual inspection are the bar; Simon's approval moves to the drafted rows and the preview Artifact.
2. **C6 attaches media across the whole cycle**, not just one row: the announcement row (`pipeline.posts` where `post_class = 'teardown'` and `first_comment` carries `/blog/<slug>`) gets the full set (`carousel_manifest`, `carousel_dir`, `carousel_slide_urls`); each derivative saved THIS run gets `format: 'image'` and one assigned slide in its `media` (`{carousel_slide_k, carousel_slide_png}`), mapped by its slice per the fan-out media plan: scored-receipts → slide 1 (cover), worst-gap / name-the-failure-mode → slide 3 or 5 (whichever the angle slices), remediation / before-after → slide 6.
3. **Preview Artifact is mandatory** (C5's preview-channel rule): publish the page with all 7 PNGs embedded as data URIs, and the item-3 ntfy ping becomes `'<N> LinkedIn drafts + carousel ready — <slug> — <artifact url>'`.
4. **Carousel failure never loses the text drafts:** any C-stage failure → log it (`status: 'error'`), keep the saved drafts, write the processed marker as usual, and the ping says `carousel failed at <stage>` so Simon knows to run `/repurpose carousel <slug>` interactively. Never retry-loop the image routes in scheduled mode.
