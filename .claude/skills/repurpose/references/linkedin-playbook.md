...
> **Operator-lane posts**: platform physics here still rule, but voice, hooks, CTA routing, comment-gating, and IG cross-posting live in `operator-playbook.md` (2026-08-03).
...
- **Engagement bait is detected and penalized.** "Comment YES if you agree," reaction polls, tag-a-friend — classifier-suppressed. Engagement pods draw reach penalties up to ~45% via reciprocal-pattern detection. (DigitalApplied engagement guide, 2026; ExpertLinked, Feb 2026) *Scope ruling (2026-08-03): both cited sources ban low-effort bait only — neither covers comment-gated REAL lead magnets. Those are allowed ≤1 in 5 posts with manual DM fulfillment (never automation tools). Full ruling: `operator-playbook.md` + `docs/research/operator-channel-research-2026-08-03.md` §2.*
