...
**Risk tier: medium (S + T + E)** — reads `public.clients`, `public.client_notes`, and `public.conversions`, writes a repo file, stamps `case_study_at`. On any failure:

```
❌ case-study-capture failed at [stage] — [error message] — client untouched (case_study_at not set), safe to retry
```

## Hard rule

**One client per fire.** Weekly schedule, router pattern. Never loop to a second client.

## Protocol

1. **Pick.** Oldest active client without a case study (unified CRM, migration 0023 — `public.leads` is frozen, never read it):
   ```bash
   python3 ~/projects/MetaArchitect/scripts/supabase-sql.py \
     "select id, name, company, memory, locale from public.clients \
      where status='active' and case_study_at is null order by created_at asc limit 1"
   ```
   Empty → print `case-study-capture: nothing to capture`, stop.

2. **Gather material.** This client's trail: their `client_notes` timeline (`select kind, title, content, occurred_at from public.client_notes where client_id = '<id>' order by occurred_at asc`), their `memory` (durable facts, including any case-study permission), `public.conversions` rows with this `client_id` (or whose title/raw_content mention the name/company), and anything the current repo/session logs verifiably record about the engagement. **Every claim in the draft must trace to this material** — no invented outcomes, no composite details, no fabricated quotes. Too thin to support a case study (< 3 concrete, sourced events/outcomes)? Say so, stamp nothing, stop — it stays in the queue for a later week.

3. **Anonymize by default.** "A [industry] owner in [region]" framing. The client is named ONLY if their `memory` or notes contain explicit permission on record; absent that, strip every identifying detail (name, company, product names, distinctive numbers).
...
6. **Stamp + ping.**
   ```bash
   python3 ~/projects/MetaArchitect/scripts/supabase-sql.py \
     "update public.clients set case_study_at = now() \
...
Never run against a real client as a test. Fixture client (`status='active'`, fake name, fake-but-marked notes), run once, verify file + `case_study_at`, then delete the fixture row AND the generated test file (don't commit it).
