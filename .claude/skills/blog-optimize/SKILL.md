...
- **No inline CTA in the body — strip one if present** (Simon, 2026-07-21). The website's `PostCTA` component ALWAYS renders the engagement CTA block (founding program → `/work-with-me`) after any `post_type='teardown'` body, overriding `cta_type`. An inline trailing `[.](/score)`-style CTA in the markdown stacks directly on top of it — the "three CTAs in a row" failure. `grep -c '](/score)' `on the body should be 0 for teardown rows; if the draft ends with a CTA sentence, delete the sentence, not just the link. (Known debt: the published Ramp body still carries an inline `/score` closer — cleanup is proposed, not authorized.)
